...
it('can soft delete and restore morphToMany relationship pivots', function () {
    ...
    MorphToManySoftDeletes::resetDeletedAtCallback();
    ...
    assertDatabaseHas('types', [
        ...
        'types_id' => 1,
        'types_type' => User::class,
        'deleted_at' => now()->subWeek(),
    ]);
});

it('can query morphToMany relationship pivots', function () {
    ...
    assertFalse(User::whereHas('user_types')->exists());

    assertTrue(User::whereHas('user_types', function ($query) {
        $query->withTrashed();
    })->exists());
});
